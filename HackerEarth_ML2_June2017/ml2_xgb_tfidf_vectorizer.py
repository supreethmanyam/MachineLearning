...
tfidf_vec = TfidfVectorizer(min_df=5, stop_words='english', max_features=5000)

train_vectors_desc = tfidf_vec.fit_transform(train['desc'])
test_vectors_desc = tfidf_vec.transform(test['desc'])

train_vectors_key = tfidf_vec.fit_transform(train['keywords'])
test_vectors_key = tfidf_vec.transform(test['keywords'])
...
for s in seeds:
    ...
    params['seed'] = s
    clf_xgb = xgb.train(params, dtrain, num_rounds, verbose_eval=False, evals=watchlist)
    if i == 1:
...
submit[['project_id', 'final_status']].to_csv('../submissions/submit_xgb_tfidf_prob.csv', index=False)
...
# submit[['project_id', 'final_status']].to_csv('../submissions/submit_xgb_tfidf_sub.csv', index=False)

np.savez('../input/train_tfidf', row=x_train.row, col=x_train.col,
         data=x_train.data, shape=x_train.shape)
np.savez('../input/test_tfidf', row=x_test.row, col=x_test.col,
